package com.liu.pojo;

public class paperquestiont {
    public String papernumber;
    public String questionnumber;
    public  String questionstyke;
    public  String questionneirong;
    public  String anwer;
    public  String xuanxianga;

    public String getPapernumber() {
        return papernumber;
    }

    public void setPapernumber(String papernumber) {
        this.papernumber = papernumber;
    }

    public String getQuestionnumber() {
        return questionnumber;
    }

    public void setQuestionnumber(String questionnumber) {
        this.questionnumber = questionnumber;
    }


    public String getQuestionneirong() {
        return questionneirong;
    }

    public void setQuestionneirong(String questionneirong) {
        this.questionneirong = questionneirong;
    }

    public String getAnwer() {
        return anwer;
    }

    public void setAnwer(String anwer) {
        this.anwer = anwer;
    }

    public String getXuanxianga() {
        return xuanxianga;
    }

    public void setXuanxianga(String xuanxianga) {
        this.xuanxianga = xuanxianga;
    }

    public String getXuanxiangb() {
        return xuanxiangb;
    }

    public void setXuanxiangb(String xuanxiangb) {
        this.xuanxiangb = xuanxiangb;
    }

    public String getXuanxiangc() {
        return xuanxiangc;
    }

    public void setXuanxiangc(String xuanxiangc) {
        this.xuanxiangc = xuanxiangc;
    }

    public String getXuanxiangd() {
        return xuanxiangd;
    }

    public void setXuanxiangd(String xuanxiangd) {
        this.xuanxiangd = xuanxiangd;
    }

    public String getXuanxiange() {
        return xuanxiange;
    }

    public void setXuanxiange(String xuanxiange) {
        this.xuanxiange = xuanxiange;
    }


    public  String  xuanxiangb;
    public  String  xuanxiangc;

    public  String xuanxiangd;
    public  String  xuanxiange;
    public  String fenshu;

    @Override
    public String toString() {
        return "paperquestiont{" +
                "papernumber='" + papernumber + '\'' +
                ", questionnumber='" + questionnumber + '\'' +
                ", questionstyke='" + questionstyke + '\'' +
                ", questionneirong='" + questionneirong + '\'' +
                ", anwer='" + anwer + '\'' +
                ", xuanxianga='" + xuanxianga + '\'' +
                ", xuanxiangb='" + xuanxiangb + '\'' +
                ", xuanxiangc='" + xuanxiangc + '\'' +
                ", xuanxiangd='" + xuanxiangd + '\'' +
                ", xuanxiange='" + xuanxiange + '\'' +
                ", fenshu='" + fenshu + '\'' +
                '}';
    }

    public String getQuestionstyke() {
        return questionstyke;
    }

    public void setQuestionstyke(String questionstyke) {
        this.questionstyke = questionstyke;
    }

    public String getFenshu() {
        return fenshu;
    }

    public void setFenshu(String fenshu) {
        this.fenshu = fenshu;
    }
}
